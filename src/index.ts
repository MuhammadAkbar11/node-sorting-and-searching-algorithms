...
function sorting(array: number[]): number[] {
  ...
  const left: number[] = []; // array untuk menyimpan elemen yang lebih kecil dari pivot
  const right: number[] = []; // array untuk menyimpan elemen yang lebih besar dari pivot
...
function searching(array: number[], key: number): number {
